...
public class UserInputLog implements Parcelable{
    ...
    public String getDelay(){
        ...
        try{
            dateCT = sdf.parse(getConfirmationtime());
        } catch (Exception e) {
            //The handling for the code
        }

        try {
            datePBIT = sdf.parse(getProcessBlendInTime());
        } catch (Exception e) {
            //The handling for the code
        }

        if(dateCT != null && datePBIT != null) {
            diffInMillisec = dateCT.getTime() - datePBIT.getTime();
        }
